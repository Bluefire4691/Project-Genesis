...
| — | **Python for orchestration; C++ for compute (via process boundary)** | Every major ML framework is a Python API over a C++/CUDA core (PyTorch→libtorch, JAX→XLA, vLLM→CUDA kernels); Python was never on the hot path. In this system ~95% of wall clock is inside `llama-server` (C++/GPU) in a *separate process*, with SQLite (C) and NumPy/BLAS (C/Fortran) taking most of the rest — Genesis's own Python is ~1%. **Rust was considered and rejected for now:** Burn reaches ~92–98% of PyTorch performance and Candle removes the GIL from production, but reimplementing sqlite-vec, HDBSCAN, LightGBM and the NLI runner is months of solo work to speed up the part that isn't slow. Decisive argument: **v1 failed on extraction quality and missing evaluation, not on execution speed** — a faster language reproduces that failure faster. Acknowledged Python costs: the GIL (already paid for in v1's freeze bugs), Windows deployment pain, and long-run memory behavior. **The hedge is the 0.2 backend interfaces** — plain data crosses them, so a Rust core could implement `LLMBackend`/`EmbeddingBackend` later with nothing above changing. Revisit if the agent ships as a product rather than an experiment. |
